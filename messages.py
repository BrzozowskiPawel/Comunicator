...
def listening(SERVER, PORT):
    print(text_colors.CBLUEBG2 + text_colors.CBOLD + "[LISTENING]" + text_colors.CEND + text_colors.CBLUE + " server is listening on "+ text_colors.CBOLD +f"{SERVER}" + text_colors.CEND  + text_colors.CBLUE + " on port: "  + text_colors.CBOLD +f"{PORT}." + text_colors.CEND)


def active_connections(active_connection_num):
    print(text_colors.CVIOLETBG + text_colors.CBOLD + "[ACTIVE CONNECTIONS]"+ text_colors.CEND  + text_colors.CVIOLET2 +f" {active_connection_num}" + text_colors.CEND)
...
def IP_confing_info():
    print(text_colors.CBLUE2 + "Please enter the IP address on which the server is running." + text_colors.CEND)